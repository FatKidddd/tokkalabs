...
test("createTxnFeeFromSwapEvent works with sample swapEvent and binance data", async () => {
  const swapEventSample: EventLog = {
    ...
    blockHash:
      "0x885b8ab831d7b3860872035194998b200dfb54048a680e015ed3f1374bbb110e",
    ...
    transactionHash:
      "0xb010f6b0d74657c5e4735b46b451b307b4f0ff7b927b9b1a2042fb2aea634c8e",
  ...
  const latestETHUSDTSample: [number, number] = [2650, curTime]

  const txnFee = await createTxnFeeFromSwapEvent(
    swapEventSample,
    latestETHUSDTSample,
  )

  expect(txnFee).toEqual({
